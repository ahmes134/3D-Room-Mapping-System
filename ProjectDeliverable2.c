...
{
	//button_presses +=1; 
	//sprintf(printf_buffer,"Measurement %d\r\n", button_presses); 
	//UART_printf(printf_buffer);
	
	FlashLED2(1);											// Flash the LED D2 once
	GPIO_PORTJ_ICR_R = 0x02; 					// Acknowledge flag by setting proper bit in ICR register
	//control the rotation of the stepper motor (CW rotation, 45 degree steps)
		//Pin configuration
		//IN4 (PH0) & IN1 (PH3) CREATES CLOCKWISE CONFIGURATION
		//IN4 (PH3) & IN1 (PH0) CREATES COUNTERCLOCKWISE CONFIGURATION
		for(int i=1; i<=512; i++)
		{ 
...
//Bus speed should be 24 MHz - Don't worry about it
